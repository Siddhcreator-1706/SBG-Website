...
const AdminClubs: React.FC = () => {
  ...
  const handleExportRoster = async () => {
    setIsExporting(true);
    try {
      const data = await apiRequest<ExportClubMember[]>(
        "/api/admin/club-members/all",
        { auth: true },
      );
  ...
  const handleExportSingleClub = async (club: ApiClub) => {
    ...
    try {
      const data = await apiRequest<ExportClubMember[]>(
        `/api/club-members?clubId=${club.id}`,
        { auth: true },
      );
  ...
  const fetchClubs = async () => {
    setIsLoading(true);
    try {
      const data = await apiRequest<ApiClub[]>("/api/admin/clubs", {
        auth: true,
      });
      setClubs(data);
    } catch (err) {
  ...
  const handleViewEvents = async (club: ApiClub) => {
    ...
    try {
      const data = await apiRequest<AppEvent[]>(
        `/api/admin/clubs/${club.id}/events`,
        { auth: true },
      );
      setClubEvents(data);
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-foreground tracking-tighter leading-tight">
            Manage Clubs
          </h1>
          <p className="text-textSecondary mt-2 text-base font-medium">
            View, edit, or remove clubs from the system.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[800px] text-left text-sm">
            <thead className="bg-hoverSoft border-b border-borderSoft uppercase tracking-wider text-xs font-semibold text-textMuted text-center">
              <tr>
                <th className="px-3 py-2 sm:py-4 w-[25%] text-center">
                  Club Name
                </th>
                <th className="px-3 py-2 sm:py-4 w-[20%] text-center">Email</th>
                <th className="px-3 py-2 sm:py-4 w-[20%] text-center">
                  Category
                </th>
                <th className="px-3 py-2 sm:py-4 w-[35%] text-center">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-borderSoft">
              {isLoading ? (
                ...
                <tr>
                  <td
                    colSpan={4}
                    className="px-3 py-2 sm:py-12 text-center text-textMuted"
                  >
                    No clubs found.
                  </td>
                </tr>
                ...
                paginatedClubs.map((club) => (
                  ...
                  >
                    <td className="px-3 py-2 sm:py-4 font-medium text-textPrimary">
                      {club.name}
                    </td>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between px-4 py-3 sm:px-6 border-t border-borderSoft bg-card gap-4">
            <div className="flex items-center text-sm text-textMuted">
              ...
              <span className="font-medium mx-1">
                {Math.min(startIndex + itemsPerPage, filteredClubs.length)}
              </span>{" "}
              of{" "}
              <span className="font-medium mx-1">{filteredClubs.length}</span>{" "}
              results
            </div>
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new club account. The email must end with @dau.ac.in.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setAddFormData({ ...addFormData, groupCategory: v })
                }
              >
                <SelectTrigger
                  id="add-category"
                  className="w-full h-10 border-borderSoft bg-transparent"
                >
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setAddFormData({ ...addFormData, organizationType: v })
                }
              >
                <SelectTrigger
                  id="add-org-type"
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Update the club's information below.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setEditFormData({ ...editFormData, groupCategory: v })
                }
              >
                <SelectTrigger
                  id="category"
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setEditFormData({ ...editFormData, organizationType: v })
                }
              >
                <SelectTrigger
                  id="org-type"
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong className="text-textPrimary">{clubToDelete?.name}</strong>
              ? This will permanently remove the club profile and{" "}
              <strong>ALL of its event bookings</strong>. This action cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
      ...
      <Sheet open={eventsSheetOpen} onOpenChange={setEventsSheetOpen}>
        <SheetContent
          side="right"
          className="w-full sm:max-w-md lg:max-w-lg overflow-y-auto bg-background border-l border-borderSoft"
        >
          ...
          <div className="space-y-4">
            {isLoadingEvents ? (
              ...
              clubEvents.map((event) => {
                ...
                const endDate = event.dynamic_end_date
                  ? new Date(event.dynamic_end_date)
                  : new Date(startDate);
                endDate.setHours(23, 59, 59, 999);

                let timelineStatus = "Ongoing";
                let badgeVariant: "success" | "outline" | "secondary" =
                  "success";
                if (now < startDate) {
                ...
                return (
                  <Card
                    key={event.id}
                    className="rounded-lg bg-card border border-borderSoft hover:border-brand/50 transition-colors shadow-sm"
                  >
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-semibold text-textPrimary leading-tight pr-4">
                          {event.name}
                        </h4>
                        <Badge
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm mt-3">
                        <div>
                          <span className="text-textMuted text-xs block uppercase tracking-wider mb-0.5">
                            Date
                          </span>
                        ...
                        <div>
                          ...
                          <div
                            className="font-medium text-textSecondary truncate max-w-full"
                            title={event.venue}
                          >
                            {event.venue}
                          </div>
