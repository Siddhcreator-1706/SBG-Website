...
const AddBookingDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onCreated,
}) => {
  ...
  const [bookingType, setBookingType] = useState<"recurring" | "continuous">(
    "recurring",
  );
  ...
  useEffect(() => {
    if (selectedClubId) {
      apiRequest<any[]>(`/api/admin/clubs/${selectedClubId}/events`, {
        auth: true,
      })
        .then(setClubEvents)
        .catch(() => setClubEvents([]));
      ...
      if (!isMeeting && selectedEventId) {
        ...
        if (linked) {
          ...
          for (const slot of timeSlots) {
            ...
            if (end > eventEnd) {
              setError(
                `Cannot book venue slot after the event ends (${eventEnd.toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}).`,
              );
              return;
            }
          }
      ...
      <DialogContent className="sm:max-w-[550px] max-h-[90dvh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="grid gap-2">
            <Label
              htmlFor="organizing-club"
              className={cn(submitted && !selectedClubId && "text-error")}
            >
              Organizing Club
            </Label>
            <Select
          ...
          <div className="grid gap-2">
            <Label
              ...
              className={cn(
                submitted && !selectedEventId && !isMeeting && "text-error",
              )}
            >
              Select Event
            </Label>
            ...
            >
              <SelectTrigger
                ...
                className={cn(
                  "bg-card",
                  submitted && !selectedEventId && !isMeeting
                    ? "border-error ring-1 ring-error/30"
                    : "",
                )}
              >
              ...
              <SelectContent>
                {clubEvents
                  .filter((evt) => {
                    const d = new Date(
                      evt.dynamic_end_date || evt.end_date || evt.date,
                    );
                ...
                {clubEvents.length === 0 && (
                  <div className="px-2 py-3 text-sm text-textMuted text-center">
            ...
            <div
              ...
              onClick={() => {
                if (!selectedEventId) setIsMeeting((prev) => !prev);
              }}
              role="checkbox"
              ...
              onKeyDown={(e) => {
                if (!selectedEventId && (e.key === " " || e.key === "Enter")) {
                  e.preventDefault();
                ...
                <div
                  className={cn(
                    ...
                    isMeeting
                      ...
                      : "border-textMuted/40 bg-transparent",
                  )}
                >
          ...
          <div className="grid gap-2">
            <Label
              ...
              className={cn(
                submitted &&
                  (!bookingName || bookingName.trim().length === 0) &&
                  "text-error",
              )}
            >
              Booking Name
            ...
            <Input
              ...
              className={cn(
                submitted && (!bookingName || bookingName.trim().length === 0)
                  ? "border-error ring-1 ring-error/30"
                  : "",
              )}
            />
          ...
          <div className="grid gap-2">
            <Label
              htmlFor="booking-venues"
              className={cn(
                submitted && selectedVenues.length === 0 && "text-error",
              )}
            >
              Venues
            </Label>
            <div
              className={cn(
                "max-h-36 overflow-y-auto rounded-md border p-2 space-y-1 bg-card",
                submitted && selectedVenues.length === 0
                  ? "border-error ring-1 ring-error/30"
                  : "border-borderSoft",
              )}
            >
              {venues.length === 0 ? (
                <p className="text-xs text-textMuted py-2 text-center">
                  Loading venues...
                </p>
              ) : (
                venues.map((v) => (
          ...
          <div className="grid gap-2">
            ...
            <div
              className={cn(
                "grid gap-3 p-3 rounded-md border bg-card",
                submitted && (!startDate || !endDate || !startTime || !endTime)
                  ? "border-error ring-1 ring-error/30"
                  : "border-borderSoft",
              )}
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label
                    htmlFor="add-booking-start-date"
                    className={cn(
                      "text-xs",
                      submitted && !startDate
                        ? "text-error"
                  ...
                  <DatePicker
                    ...
                    className={cn(
                      "bg-card w-full",
                      submitted && !startDate ? "border-error" : "",
                    )}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label
                    htmlFor="add-booking-end-date"
                    className={cn(
                      "text-xs",
                      submitted && !endDate
                        ? "text-error"
                  ...
                  <DatePicker
                    ...
                    className={cn(
                      "bg-card w-full",
                      submitted && !endDate ? "border-error" : "",
                    )}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label
                    htmlFor="add-booking-start-time"
                    className={cn(
                      "text-xs",
                      submitted && !startTime
                        ? "text-error"
              ...
              {startDate &&
                ...
                startDate.getTime() !== endDate.getTime() && (
                  <div className="pt-3 border-t border-borderSoft mt-2 space-y-3">
                    <div>
                      <Label className="text-textPrimary font-bold text-sm block">
                        Multi-Day Booking Schedule
                      </Label>
                      <p className="text-xs text-textSecondary mt-0.5">
                        Select how slots should be allocated
                      </p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-bold text-xs sm:text-sm text-textPrimary leading-snug">
                            Continuous Slot
                          </div>
                          <div className="text-[11px] text-textSecondary mt-0.5 leading-tight">
                            Book continuously start to end
                          </div>
                        </div>
                      </button>
          ...
          {coCurricularWarning && (
            <div
              className={`text-sm rounded-md px-3 py-2 border ${
                coCurricularWarning.startsWith("SBG has already")
